...
process generate_report {
    ...
    try:
        with open("${read_summary}", 'r') as f:
            ...
            # Get raw and final read counts per sample
            sample_data = {}
            for line in lines:
                ...
                if len(parts) >= 3:
                    ...
                    sample_data[sample_id][stage] = reads
            
            # Calculate statistics for raw reads
            raw_reads = []
            final_reads = []
            properly_paired = []
            ...
            for sample in sample_data.values():
                if 'raw' in sample:
                    raw_reads.append(sample['raw'])
                # Get mapped reads as final
                if 'map' in sample:
                    final_reads.append(sample['map'])
                elif 'repr' in sample:
                    final_reads.append(sample['repr'])
                # Also get properly paired if available
                if 'pp' in sample:
                    properly_paired.append(sample['pp'])
