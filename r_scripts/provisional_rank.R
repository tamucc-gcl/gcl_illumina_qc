...
cv <- tryCatch(
  read_tsv(cv_file, col_names = c("id","c1","c2","sim","cv","gini"),
           col_types = "ciiddd", na = c("NA","")),
  error = function(e) NULL
)

if (nrow(cheap) == 0) {
  writeLines("id\tc1\tc2\tsim\tn_contigs\tcv\tgini\tdist_inflect\trank_nb\trank_cv\trank_gini\trank_anchor\tagg_rank\tprovisional_order",
             "provisional_rank.tsv")
  writeLines(character(0), "survivors.txt"); quit(save="no", status=0)
}

# join CV/Gini onto the candidate table by id (keep only id,cv,gini from cv)
if (!is.null(cv) && nrow(cv) > 0) {
  d <- left_join(cheap, cv %>% select(id, cv, gini), by = "id")
} else {
  d <- cheap %>% mutate(cv = NA_real_, gini = NA_real_)
}
...
# ---- 2 coverage uniformity: CV and Gini (lower better) ----
# rank() puts NA last; replace all-NA with NA rank (signal dropped if no CV at all)
...
rank_cols <- c("rank_nb","rank_cv","rank_gini","rank_anchor")
d$agg_rank <- rowMeans(d[, rank_cols], na.rm = TRUE)

ranked <- d %>%
  arrange(agg_rank, cv, id) %>%
  mutate(provisional_order = row_number())

write_tsv(
  ranked %>% select(id, c1, c2, sim, n_contigs, cv, gini, dist_inflect,
                    rank_nb, rank_cv, rank_gini, rank_anchor,
...
cat(sprintf("provisional_rank: %d candidates | nb_cutoff1=%s | anchor=%s | CV signal=%s | survivors=%d\n",
            ...
            ifelse(any(is.finite(d$cv)), "active", "MISSING"), n_take))
